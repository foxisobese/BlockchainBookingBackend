...
@app.route(
    "/set_address", methods=["POST"]
)  # owner address 설정 (임시로 flask_sqlalchemy로 DB)
def set_address():
...
@app.route(
    "/transfer_tokens", methods=["POST"]
)  # 특정 토큰에 대해 From, To의 username을 받아 transfer
...
def mint_tickets_route():
    ...
    transfer_tickets(
        web3, instance, eventName, eventDate, seatNumber, price, newTokenUrl
    )
